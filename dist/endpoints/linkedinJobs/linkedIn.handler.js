...
exports.updateLinkedInJobHandler = exports.updateCarrierPathLinkJobHandler = exports.getAllLinkedInHandler = exports.fetchLinkedInJobs = exports.deleteLinkedInJobHandler = exports.deleteLinkedInAutomationJobHandler = exports.createLinkedInJobHandler = void 0;
var _uuid = require("uuid");
var _playwright = require("playwright");
var _bigquery = require("../../config/bigquery");
var _linkedIn = require("../../queries/linkedIn/linkedIn.queries");
var _audit = require("../../queries/audit/audit.queries");
var _linkedInJobStorage = require("../../config/linkedInJobStorage");
var _dateFns = require("date-fns");
const TABLE_LINKEDIN_JOBS = process.env.TABLE_LINKEDIN_JOBS;

// Function to check if the linkedIn table exists
const checkLinkedInTableExists = async () => {
  try {
...
const createLinkedInTableIfNotExists = async () => {
...
const createLinkedInJobHandler = async (jobData, file) => {
  const {
    title,
    company,
    link,
  ...
  try {
    ...
    const [existingJob] = await _bigquery.bigquery.query({
      query: `
      ...
      params: {
        entityURN
      } // Ensure correct param structure
    });
    ...
    if (file) {
      ...
      uploadedImgUrl = await (0, _linkedInJobStorage.uploadLinkedInImageToGCS)(file.buffer, fileName, file.mimetype);
    }
    console.log('Inserting job info into BigQuery...');
    await _bigquery.bigquery.query({
      query: _linkedIn.linkedInQueries.createLinkedInJob,
      params: {
        id: jobId,
        title,
        company,
        link,
        carrierPathLink: carrierPathLink || '',
        // Ensure this is defined
        jobTrigger: 'Manual',
        // Set to 'Manual' for manual jobs
        location,
        datePosted,
        entityURN,
        // Use provided entityURN (not uuidv4())
...
//Entity URN Checking
exports.createLinkedInJobHandler = createLinkedInJobHandler;
const getExistingEntityURNs = async () => {
  try {
    const query = `
    ...
    return new Set(rows.map(row => row.entityURN));
  } catch (error) {
...
function convertRelativeDate(relativeDate) {
  ...
  if (match) {
    ...
    let exactDate;
    if (unit === 'day') {
      exactDate = (0, _dateFns.subDays)(now, value);
    } else if (unit === 'week') {
      exactDate = (0, _dateFns.subWeeks)(now, value);
    } else if (unit === 'month') {
      exactDate = (0, _dateFns.subMonths)(now, value);
    } else if (unit === 'year') {
      exactDate = (0, _dateFns.subYears)(now, value);
    } else {
      return (0, _dateFns.format)(now, 'yyyy-MM-dd'); // Default to today if unknown
    }
    return (0, _dateFns.format)(exactDate, 'yyyy-MM-dd'); // Convert to "YYYY-MM-DD"
  }
  return (0, _dateFns.format)(now, 'yyyy-MM-dd'); // Default to today if format is unknown
}
const fetchLinkedInJobs = async () => {
  console.log('Deleting all jobs with jobTrigger: Automation');
  if (!(await checkLinkedInTableExists())) throw new Error(`Table '${TABLE_LINKEDIN_JOBS}' does not exist.`);

  // Delete all jobs where jobTrigger = 'Automation'
  const deleteJobBoard = await _bigquery.bigquery.query({
    query: _linkedIn.linkedInQueries.deleteOldAutomationJobs
  });
  console.log('Deleted Data', JSON.stringify(deleteJobBoard));
  ...
  // ];
  const jobRoles = ['Data Scientist', 'Junior Data Scientist', 'Senior Data Scientist', 'Lead Data Scientist', 'Principal Data Scientist', 'Data Science Consultant', 'Data Science Manager', 'Applied Data Scientist', 'AI Scientist', 'Machine Learning Engineer', 'Senior Machine Learning Engineer', 'Lead Machine Learning Engineer', 'Principal Machine Learning Engineer', 'Machine Learning Researcher', 'AI Engineer', 'AI Specialist', 'AI Research Scientist', 'NLP Engineer', 'Computer Vision Engineer', 'Deep Learning Engineer', 'AI Product Manager', 'AI Solutions Architect', 'AI Consultant', 'Associate Data Scientist', 'Research Data Scientist', 'Data Science Engineer', 'Computational Data Scientist', 'Statistical Data Scientist', 'Machine Learning Developer', 'AI/ML Engineer', 'Applied Machine Learning Engineer', 'AI Algorithm Engineer', 'Data Engineer', 'Senior Data Engineer', 'Lead Data Engineer', 'Principal Data Engineer', 'Big Data Engineer', 'Cloud Data Engineer', 'ETL Developer', 'Data Architect', 'Database Engineer', 'Data Infrastructure Engineer', 'DataOps Engineer', 'Data Pipeline Engineer', 'Hadoop Engineer', 'Snowflake Engineer', 'Data Platform Engineer', 'Data Engineering Consultant', 'Data Pipeline Developer', 'ETL Engineer', 'Big Data Consultant', 'Spark Engineer', 'Cloud Data Solutions Engineer', 'Data Analyst', 'Junior Data Analyst', 'Senior Data Analyst', 'Data Science Analyst', 'Business Data Analyst', 'Quantitative Data Analyst', 'Decision Science Analyst', 'Data Insights Analyst', 'Reporting Analyst', 'BI Data Analyst', 'BI Reporting Analyst', 'Data Visualization Analyst', 'Marketing Data Analyst', 'Financial Data Analyst', 'Product Data Analyst', 'Risk Data Analyst', 'Healthcare Data Analyst', 'Customer Insights Analyst', 'Operations Data Analyst', 'HR Data Analyst', 'Workforce Analyst', 'Pricing Analyst', 'Fraud Analyst', 'Predictive Analytics Specialist', 'Operations Research Analyst', 'Data Strategy Analyst', 'Insights & Analytics Manager', 'Business Intelligence Consultant', 'ETL Analyst', 'Big Data Analyst', 'Power BI Developer', 'Tableau Developer', 'Generative AI Engineer', 'AI Prompt Engineer', 'AI Solutions Engineer', 'AI Developer', 'AI Research Engineer', 'AI Data Engineer', 'AI Product Manager', 'AI Content Creator', 'AI Chatbot Developer', 'AI Ethics Specialist', 'AI Prompt Specialist', 'AI Application Engineer', 'Conversational AI Engineer', 'AI Data Scientist', 'AI/ML Consultant', 'AI System Engineer', 'LLM Engineer', 'AI Automation Engineer', 'AI Innovation Engineer', 'ML Engineer (Generative AI)', 'Foundation Model Engineer', 'Data Labeling Specialist', 'Domo Developer', 'Domo Analyst', 'Domo Consultant', 'Domo Engineer', 'BI Developer (Domo)', 'Data Visualization Specialist (Domo)', 'Data Visualization Engineer', 'Dashboard Developer', 'BI & Reporting Specialist', 'Data Dashboard Designer'];

  // ✅ Define locations inside the function
  const locations = ['India'];

  // Set to track unique entityURNs during scraping
  const scrapedEntityURNs = new Set();
  try {
    for (const role of jobRoles) {
      for (const location of locations) {
        ...
        while (hasMoreJobs && currentPage < 5) {
          // while (hasMoreJobs) {
          // Prevent infinite loop
          const LINKEDIN_URL = `https://www.linkedin.com/jobs/search/?keywords=${encodeURIComponent(role)}&location=${encodeURIComponent(location)}&start=${currentPage * 20}`;
          try {
            console.log(`Navigating to: ${LINKEDIN_URL}`);
            await page.goto(LINKEDIN_URL, {
              waitUntil: 'networkidle',
            ...
            for (const card of jobCards) {
              const job = await card.evaluate(node => ({
                title: node.querySelector('.base-search-card__title')?.textContent?.trim() || 'No title',
                company: node.querySelector('.base-search-card__subtitle')?.textContent?.trim() || 'No company',
                link: node.querySelector('.base-card__full-link')?.getAttribute('href') || 'No link',
                location: node.querySelector('.job-search-card__location')?.textContent?.trim() || 'No location',
                datePosted: node.querySelector('.job-search-card__listdate')?.textContent?.trim() || 'No date',
                entityURN: node.getAttribute('data-entity-urn')?.trim() || '',
                // Ensure no empty URNs
                imgSrc: node.querySelector('.artdeco-entity-image')?.getAttribute('src') || 'No image',
                jobDescription: node.querySelector('.job-search-card__desc')?.textContent?.trim() || 'No description'
              }));
            ...
            console.error(`Error loading page for "${role}" in "${location}":`, error);
            hasMoreJobs = false;
          }
          await page.waitForTimeout(5000); // Avoid detection
        }
...
const storeJobsInBigQuery = async jobs => {
  await createLinkedInTableIfNotExists();
  try {
    ...
    for (const job of uniqueJobs) {
      await _bigquery.bigquery.query({
        query: _linkedIn.linkedInQueries.insertLinkedInJob,
        params: {
          id: (0, _uuid.v4)(),
          title: job.title,
          company: job.company,
...
const getAllLinkedInHandler = async () => {
  await checkLinkedInTableExists();
  try {
    console.log('Fetching all LinkedIn Datas...');
    const [rows] = await _bigquery.bigquery.query({
      query: _linkedIn.linkedInQueries.getAllLinkedInJobs
    });
...
exports.getAllLinkedInHandler = getAllLinkedInHandler;
const updateLinkedInJobHandler = async (company, file) => {
  try {
    ...
    const [linkedInResults] = await _bigquery.bigquery.query({
...
exports.updateLinkedInJobHandler = updateLinkedInJobHandler;
const updateCarrierPathLinkJobHandler = async (id, updatedData) => {
  try {
    ...
    if (!Array.isArray(linkedInResults) || linkedInResults.length === 0) {
      ...
      return {
        success: false,
        message: `No job found for ID ${id}.`
      };
    }
...
exports.updateCarrierPathLinkJobHandler = updateCarrierPathLinkJobHandler;
const deleteLinkedInJobHandler = async (id, req) => {
  const {
    user
  } = req;
  try {
    ...
    const deleteJobBoard = await _bigquery.bigquery.query({
      query: _linkedIn.linkedInQueries.deleteJobBoard,
      params: {
        jobId: id
      } // ✅ Fix: Match the parameter name
    });
...
exports.deleteLinkedInJobHandler = deleteLinkedInJobHandler;
const deleteLinkedInAutomationJobHandler = async req => {
  const {
    user
  } = req;
  try {
    ...
    // Delete all jobs where jobTrigger = 'Automation'
    const deleteJobBoard = await _bigquery.bigquery.query({
      query: _linkedIn.linkedInQueries.deleteAllAutomationJobs
    });
...
exports.deleteLinkedInAutomationJobHandler = deleteLinkedInAutomationJobHandler;
//# sourceMappingURL=linkedIn.handler.js.map