...
exports.updateClassHandler = exports.getClassByModuleIdHandler = exports.getClassByIdHandler = exports.getAllClassesHandler = exports.deleteClassHandler = exports.createClassHandler = void 0;
var _uuid = require("uuid");
var _bigquery = require("../../config/bigquery");
var _class = require("../../queries/class/class.queries");
var _classStorage = require("../../config/classStorage");
...
const checkClassTableExists = async () => {
  try {
...
const createClassTableIfNotExists = async () => {
...
const createClassHandler = async (req, classData, files) => {
  const {
    classTitle,
    classDescription,
    moduleId,
  ...
  try {
    ...
    const [existingClass] = await _bigquery.bigquery.query({
    ...
    if (existingClass.length > 0) {
      ...
      return {
        success: false,
        message: "Class Title already exists."
      };
    }
    console.log("Creating new Class...");
    const classId = (0, _uuid.v4)();
    ...
    if (files?.materialForClass) {
      try {
        ...
        uploadedMaterialUrl = await (0, _classStorage.uploadMaterialForClassToGCS)(file.buffer, fileName, file.mimetype);
      } catch (uploadError) {
    ...
    if (files?.assignmentFile) {
      try {
        ...
        uploadedAssignmentUrl = await (0, _classStorage.uploadMaterialForClassToGCS)(file.buffer, fileName, file.mimetype);
      } catch (uploadError) {
    ...
    await _bigquery.bigquery.query({
      query: _class.classQueries.createClass,
      params: {
        id: classId,
        classTitle,
        classDescription,
        moduleId,
        assignmentName,
        assignmentFile: uploadedAssignmentUrl,
        // Storing the assignment file URL
        materialForClass: uploadedMaterialUrl,
        // Storing the material file URL
        totalMarks,
        createdBy: user?.id,
        createdAt
      }
...
const getAllClassesHandler = async () => {
  ...
  try {
    ...
    const [rows] = await _bigquery.bigquery.query({
      query: _class.classQueries.getAllClasses
    });
...
const getClassByIdHandler = async classId => {
  await checkClassTableExists();
  try {
...
const getClassByModuleIdHandler = async moduleId => {
  await checkClassTableExists();
  try {
    console.log(`Fetching classes for module ID: ${moduleId}`);
    const [rows] = await _bigquery.bigquery.query({
      query: _class.classQueries.getClassByModuleId,
      params: {
        moduleId
      }
    });
...
exports.getClassByModuleIdHandler = getClassByModuleIdHandler;
const updateClassHandler = async (req, id, updatedData, files) => {
  try {
    const {
      user
    } = req;
    ...
    const [classResults] = await _bigquery.bigquery.query({
    ...
    if (files?.materialForClass?.length) {
      ...
      if (classData.materialForClass) {
        ...
        if (oldFileName) {
          await (0, _classStorage.deleteClassFilesFromGCS)(oldFileName);
        }
      }
      ...
      newMaterialForClassUrl = await (0, _classStorage.uploadMaterialForClassToGCS)(materialFile.buffer, materialFileName, materialFile.mimetype);
    ...
    if (files?.assignmentFile?.length) {
      ...
      if (classData.assignmentFile) {
        ...
        if (oldAssignmentFileName) {
          await (0, _classStorage.deleteClassFilesFromGCS)(oldAssignmentFileName);
        }
      }
      ...
      newAssignmentFileUrl = await (0, _classStorage.uploadMaterialForClassToGCS)(assignmentFile.buffer, assignmentFileName, assignmentFile.mimetype);
    ...
    await _bigquery.bigquery.query({
      query: _class.classQueries.updateClass,
      params: queryParams
    });
...
const deleteClassHandler = async (req, id) => {
  const {
    user
  } = req;
  try {
    ...
    if (fileName) {
      ...
      await (0, _classStorage.deleteClassFilesFromGCS)(fileName);
    }
