...
exports.updateCourseCategoryHandler = exports.getCourseCategoryByIdHandler = exports.getAllCourseCategoriesHandler = exports.deleteCourseCategoryHandler = exports.createCourseCategoryHandler = void 0;
var _uuid = require("uuid");
var _bigquery = require("../../config/bigquery");
var _courseCategory = require("../../queries/courseCategory/courseCategory.queries");
var _courseCategoryStorage = require("../../config/courseCategoryStorage");
...
const checkCourseCategoryTableExists = async () => {
  try {
...
const createCourseCategoryTableIfNotExists = async () => {
...
const createCourseCategoryHandler = async (req, courseCategoryData, file) => {
  try {
    const id = (0, _uuid.v4)();
    const {
      user
    ...
    await createCourseCategoryTableIfNotExists();
    console.log("Checking if course category name already exists...");
    const [existingCourseCategory] = await _bigquery.bigquery.query({
      query: `SELECT id FROM \`teqcertify.lms.${process.env.TABLE_COURSE_CATEGORY}\` WHERE coursecategoryName = @coursecategoryName`,
      params: {
        coursecategoryName
      },
    ...
    console.log("Creating new course category...");
    const courseCategoryId = (0, _uuid.v4)();
    const createdAt = new Date().toISOString();
    const updatedAt = createdAt;
    let uploadedImgUrl = "";
    ...
    if (file) {
      try {
        const fileName = `${courseCategoryId}.${file.mimetype.split("/")[1]}`;
        console.log("Uploading course category image to GCS...");
        uploadedImgUrl = await (0, _courseCategoryStorage.uploadCourseCategoryImageToGCS)(file.buffer, fileName, file.mimetype);
      } catch (uploadError) {
        console.error("Error uploading image to GCS:", uploadError);
        return {
          success: false,
    ...
    console.log("Inserting course category info into BigQuery...");
    await _bigquery.bigquery.query({
      query: _courseCategory.queries.createCourseCategory,
      params: {
        id: courseCategoryId,
        coursecategoryName,
        description,
...
exports.createCourseCategoryHandler = createCourseCategoryHandler;
const getAllCourseCategoriesHandler = async () => {
  await checkCourseCategoryTableExists();
  try {
    console.log('Fetching all course categories...');
    const [rows] = await _bigquery.bigquery.query({
      query: _courseCategory.queries.getAllCourseCategories
    });
...
exports.getAllCourseCategoriesHandler = getAllCourseCategoriesHandler;
const getCourseCategoryByIdHandler = async id => {
  await checkCourseCategoryTableExists();
  try {
    console.log(`Fetching course category with ID: ${id}`);
    const [rows] = await _bigquery.bigquery.query({
      query: _courseCategory.queries.getCourseCategoryById,
      params: {
        id
      }
    });
...
exports.getCourseCategoryByIdHandler = getCourseCategoryByIdHandler;
const updateCourseCategoryHandler = async (id, req, updatedData, file) => {
  try {
    const {
      user
    } = req;
    ...
    if (!tableExists) {
      return {
        success: false,
        message: `Table '${TABLE_COURSE_CATEGORY}' does not exist.`
      };
    }

    // Fetch existing company data
    const [courseCategoryResults] = await _bigquery.bigquery.query({
    ...
    if (file) {
      try {
        ...
        if (oldFileName) {
          await (0, _courseCategoryStorage.deleteCourseCategoryImageFromGCS)(oldFileName);
        }

        // Upload new image
        const fileName = `${id}.${file.mimetype.split("/")[1]}`;
        newCourseCategoryImgUrl = await (0, _courseCategoryStorage.uploadCourseCategoryImageToGCS)(file.buffer, fileName, file.mimetype);
      } catch (uploadError) {
        console.error("Error uploading courseCategory image:", uploadError);
        return {
          success: false,
          message: "Failed to upload courseCategory image."
        };
      }
    }
...
exports.updateCourseCategoryHandler = updateCourseCategoryHandler;
const deleteCourseCategoryHandler = async (id, req) => {
  const {
    user
  } = req;
  try {
    ...
    if (fileName) {
      ...
      await (0, _courseCategoryStorage.deleteCourseCategoryImageFromGCS)(fileName);
    }
    const deleteCategory = await _bigquery.bigquery.query({
      query: _courseCategory.queries.deleteCourseCategory,
      params: [{
        name: 'id',
        value: id
      }]
    ...
    return {
      success: true,
      message: `CourseCategory with ID ${id} deleted successfully.`
    };
  } catch (error) {
...
exports.deleteCourseCategoryHandler = deleteCourseCategoryHandler;
//# sourceMappingURL=courseCategory.handler.js.map