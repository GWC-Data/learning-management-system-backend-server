import { bucket, userProfilePicFolder } from "./storageConfig";
 
export const uploadUserProfilePicToGCS = async (fileBuffer: Buffer, fileName: string, mimeType: string): Promise<string> => {
  try {
    const filePath = `${userProfilePicFolder}/${fileName}`;
...
export const deleteUserProfilePicFromGCS = async (fileName: string): Promise<void> => {
  try {
    const filePath = `${userProfilePicFolder}/${fileName}`;
