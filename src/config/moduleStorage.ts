import { bucket, moduleFolder } from "./storageConfig";

export const uploadModuleMaterialToGCS = async (fileBuffer: Buffer, fileName: string, mimeType: string): Promise<string> => {
  try {
...
export const deleteModuleMaterialFromGCS = async (fileName: string): Promise<void> => {
  try {
    const filePath = `${moduleFolder}/${fileName}`;
