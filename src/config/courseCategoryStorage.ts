import { bucket, courseCategoryFolder } from "./storageConfig";

export const uploadCourseCategoryImageToGCS = async (fileBuffer: Buffer, fileName: string, mimeType: string): Promise<string> => {
  try {
    const filePath = `${courseCategoryFolder}/${fileName}`;
...
export const deleteCourseCategoryImageFromGCS = async (fileName: string): Promise<void> => {
  try {
    const filePath = `${courseCategoryFolder}/${fileName}`;
